...
function changeKey(key)
{
	...
	if (isP != 'S' && key != 1) { 	// && (key > 1 || key < -1)){
		$('#fancyButton1').fancybox({'frameWidth':625, 'frameHeight':315, 'url': base_href + 'site/login2.htm?func=window.top.isP=\'S\';window.top.changeKey(' + key + ')&clos=1&iframe', 'padding':5})
		$('#fancyButton1').click()
		return false
	}
...
function showDrawings()
{
	...
	if (!bolChordsOnLyrics)
	{
		...
		$(this)
		.html('<img src="http://www.cifras.com.br/imagens/c3/chordimage.asp?chord=' + chords[element.html()].variations.split(" ")[0].replaceAll(",", " ") + '" rel="' + element.html() + '">')
...
function setInversionsDiv()
{
		...
		$('#core a').bind('mouseenter', function(event) {
			...
			top1 += (isIE && ver < 8 ? (ver == 6 ? -160 : heightBanner + 5) : heightBanner + 5)
			left1 += (isIE? -14 : 0)
			left1 += (hasSuperBanner? -160: 0)
			var isPrintVersion = ($('body').css("margin-top").replace("px", "") == '0')
			top1 += (isPrintVersion ? 116 : -20 )
...
function printSong(strType)
{
	...
	//$.get('ajax.asp', 'acao=get_status', function(r){
		...
		switch (strType)
		{
			case 'rtf':
				...
				$('#prtop, #prartist').children('img:eq(2)').css('display','block').bind('click', function() {
					window.location.href='http://www.e-chords.com/pdf.asp?artist=' + cod_artist + '&title=' + cod_title + '&id=' + idSong + '&tipo=' + tipoTab + '&key=' + intKey + "&p4=" + (chordsUsed.replaceAll('#', '*').replaceAll('+', '$').replaceAll('/', '\\')) + "&p5=" + paramDigitacoes
					})
				$('#prtop, #prartist').children('img:eq(3)').css('display','block').bind('click', function() {
					window.location.href='http://www.e-chords.com/pdf.asp?artist=' + cod_artist + '&title=' + cod_title + '&id=' + idSong + '&tipo=' + tipoTab + '&tipodoc=doc&key=' + intKey + "&p4=" + (chordsUsed.replaceAll('#', '*').replaceAll('+', '$').replaceAll('/', '\\')) + "&p5=" + paramDigitacoes
					})
				break
			case 'txt':
		...
		if (isP != 'S'){
		$('#fancyButton1').fancybox({'frameWidth':625, 'frameHeight':315, 'url': base_href + 'site/login2.asp?func=printSong(\'' + strType + '\')&clos=1&iframe', 'padding':5, callbackOnClose: function() { cancelPrint();}  });
		$('#fancyButton1').click()
...
function btnSaveTo() {
	if (isLoggedIn) {
		...
		paramDigitacoes = ''
			...
			$('.opcoes >span:eq(5)').fancybox({'frameWidth':635, 'frameHeight':334, 'url': base_href + 'site/premium.asp?iframe', padding:5});
		}
	} else {
		$('.opcoes >span:eq(5)').fancybox({'frameWidth':625, 'frameHeight':315, 'url': base_href + 'site/login1.asp?func=btnSaveTo()&iframe', padding:5});
	}
}
...
function playBeat(divId, folder, songName, artistName)
{
	...
	flashvars.xmlPath = "http://www.e-chords.com/flash/audio/deploy/player/xml/settingsap.xml";
	flashvars.artistName = artistName;
	flashvars.songName = "Tempo " + songName;
	flashvars.songURL = "http://www.e-chords.com/arquivos/beats/" + folder + "/" + songName + "bpm.mp3";
	//alert("http://www.e-chords.com/arquivos/beats/" + folder + "/" + songName + "bpm.mp3")
	/** SWF OBJECT EMBED **/
	swfobject.embedSWF("http://www.e-chords.com/flash/audio/deploy/preview.swf"+cacheBuster, divId, stageW, stageH, "9.0.124", "js/expressInstall.swf", flashvars, params);
}
function hearLoop()
{
...
$(document).ready(function(){
	...
	//if ($.browser.msie) 
	  $("body").bind('copy', function(e) {
		  if (isP != 'S'){
			$('#fancyButton1').fancybox({'frameWidth':625, 'frameHeight':315, 'url': base_href + 'site/login2.htm?func=freeCopy()&clos=1&iframe', 'padding':5})
			$('#fancyButton1').click()
			return false;
			}
            })
	...
	$('.opcoes >span:eq(0)').fancybox({'frameWidth':500, 'frameHeight':620, 'url': base_href + 'arquivos/tuner/index_english.html?iframe', 'padding':5})
	...
	$('.opcoes >span:eq(3)').click(function() { 
		...
		$('.drawchord').each(
			function(){
				...
				$(this).html("<img src='http://www.e-chords.com/images/chordimagewhitemin.asp?chord=" + chord + "&name=" + arr[0].replaceAll('#', '*').replaceAll('+', '$').replaceAll('/', '\\') + ((leftHanded == true) ? "&lefthanded=1" : "") + "'>")
			}
		);
	})
	$('.opcoes >span:eq(4)').click(function() {
	...
	/*$('.opcoes >span:eq(5)').click(function() {
		...
		$('.drawchord').each(
			function(){
				...
				$(this).html("<img src='http://www.e-chords.com/images/chordimagewhitemin.asp?chord=" + chord + "&name=" + arr[0].replaceAll('#', '*').replaceAll('+', '$').replaceAll('/', '\\') + "'>")
	...
	$('.topo_cifra >p:eq(1)').mousemove(
		function(e){
			...
			if (pos >0 && pos < 65)
			{
				...
				//$('#dificulty > img:first').attr('src',  img)
	...
	*/
	...
	//clearInterval(killFloatChord);
	//$('#debug').val($('#debug').val() + 'end 0 killFloatChord\n' )

	$(this).css({'visibility':'hidden'})   })
    })
	
	/*$('#core').bind('mouseover', function(){ 
	$("#inversions").css({'visibility':'hidden'})
	})*/
	...
	.click(function () { 
		...
		$(".tom").animate({	height: 15,	width: 57 }, 'fast')			
    })
   ...
   $().mousemove(function(e){
	...
	$(window).scroll(function (event) {
		...
		//if (y > 350) {
			//posTop = $('#fixed2').position().top + $('#ytplayer').position().top
		
		/*if (y > maxY) {
			//if (y  > maxY + (370 - $('#ytplayer').position().top) ) {
				
				$('#fixed2').css({top: -(y - maxY)
				//-(y - maxY)
				 })
			}*/
			//else
			//if (isIE) 
			//	$('#ytplayer').css({top: (y - 350)})
			//else
			//	$('#ytplayer').css({top: y })
		//}
		//else
